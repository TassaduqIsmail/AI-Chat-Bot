...
import {settone} from '../Store/Lang';

const WritingTone = ({navigation}) => {
  ...
  useEffect(() => {
    const Handlepress = async () => {
      ...
      await axios
        .get(
          `https://professionista-ai.com/wp-json/proff_ai/v1/writing-tone?token=3h97Cu0RAb4AKrDo`,
        )
        .then(i => {
  ...
  const lang = useSelector(state => state.auth.tone);
  const [select, setSelect] = useState(lang);

  const dispatch = useDispatch();
  const handlelang = item => {
    dispatch(settone(item.prompt_name));
      ...
      <View
        ...
        <View>
          <Text
            ...
            {isLanguage === 'eng'
              ? 'Selezionare il tono di scrittura'
              : 'Select writing tone'}
          </Text>
        </View>
      </View>
